...
export function getStockGroupKey(entry: StockEntry): string {
  ...
  switch (entry.type) {
    ...
    case "maderas":
      return `${base}|${entry.descripcion.toLowerCase()}|${entry.tipoMadera}|${entry.anchoCm}|${entry.largoCm}`;
    case "cano":
      return `${base}|${entry.descripcion.toLowerCase()}|${entry.largoCm}`;
    case "herramientas":
  ...
  return Array.from(groups.entries())
    .map(([key, groupEntries]) => {
      ...
      const totalQuantity = sorted.reduce(
        (sum, entry) => sum + getDisplayStockQuantity(entry),
        0,
      );
      const totalCantidadUsada = sorted.reduce(
        (sum, entry) => sum + getDisplayCantidadUsada(entry),
        0,
      );
